...
%     z_new = z_n;
    ...
    actions = struct('source_node', idx_current,...
        ...
        'primitive',[prim.name,'.trigger(ON)'],...
...
%     end
    node = plot(z_new_visual(1),z_new_visual(2),'y*','linewidth',4);
...
%     plot_edges{1,T.nnodes} = edge;
    %     keyboard
    figure(fig_xy)
    node = plot3(z_new_visual(1),z_new_visual(2),z_new_visual(5),'y*','linewidth',4);
