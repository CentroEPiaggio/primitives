...
u_lb = lower_control_bound; % lower bound
u_ub = upper_control_bound; % upper bound
...
min_increment = 0.1; % sec
max_increment = 1; % sec
steps_increment = max(round(min_increment/Ts),1); % increments of 1 seconds
steps_old = steps;
tic
while(exitflag ~= 1)
    steps = steps_old + steps_increment;
    ...
    u_unconstrained = pinv(Ad*Rmat)*(xf - Ad^(steps+1)*x0 - Bd*up);
    u_unconstrained = flipud(u_unconstrained);
    u_unconstrained = [u_unconstrained];
    
%     [u,~,~,exitflag] = lsqlin(Ccost,d_cost,H_matrix,O_matrix,Aeq,beq,lb,ub,u_unconstrained,optiontslsqlin);
        [u,~,~,exitflag] = lsqlin(Ccost,d_cost,[],[],Aeq,beq,lb,ub,[],optiontslsqlin);
    %     if ~exitflag
    steps_old = steps;
    %     else % found a feasible solution
    %         if steps_increment <= min_increment
    % %             break;
    %         else
    %             % steps_old = steps_old
    %         steps_increment = round(steps_increment/2);
    %         end
    %     end
end
disp(['Tempo trascorso in min_jerk_trajectory_loop: ' num2str(toc)])
u = flipud(u);
u = [u; up];
...
f=figure; plot(time_largesampling,states); grid on; hold on;
plot(time_largesampling(end)/2*[1 1],[-1 1]*max(abs(states(3,:))),'k')
keyboard
% close(f);
